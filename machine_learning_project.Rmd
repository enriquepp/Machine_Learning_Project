...
##Cleaning data  
We can find 159 variables in the data. Some of them not related with classe, others classified as factors, and others with more than 90% of NAs values.  
...
sum(apply(data, 2, function(i) {sum(is.na(i))}))
sum(apply(validation, 2, function(i) {sum(is.na(i))}))
```
...
The best method is **Random Forest** without Cross Validation. The reason for this result is that Random Forest doesn't need a cross validation resampling, and using it implies a smaller training dataset and none advantage.
